    ...
    jQuery.ajax({
      ...
      success: function (res) {
        if (res == "email_present") {
          jQuery("#form-messege").html("Email already present");
        } else {
          jQuery("#form-messege").html("Thank You For Registering");
        }
      },
    });
  }
...
function login() {
  ...
  var password = jQuery("#l_password").val();
  var is_error = "";
  ...
  if (!email) {
    ...
    jQuery.ajax({
      ...
      success: function (res) {
        if (res == "wrong") {
          jQuery("#l_form-messege").html("Invalid Credientials");
        } else {
